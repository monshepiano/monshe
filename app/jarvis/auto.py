...
_DEFER_RE = re.compile(
    ...
    r"\b(?:позже|потом|завтра)\b.{0,60}"
    r"\b(?:сделай|выполни|подготовь|собери|напиши|пришли|вернись)\b|"
    r"\bотложи\b.{0,40}\b(?:задач|работ|это))"
)


def should_background(text: str) -> Dict[str, Any]:
    """Маршрутизирует только ЯВНЫЙ фоновый intent пользователя.

    Тема, длина текста и предполагаемая длительность работы здесь намеренно не
    учитываются. Создание игры, большой файл или исследование выполняются в
    текущем диалоге, пока пользователь прямо не попросил фон/срок/мониторинг.
    """
    t = re.sub(r"\s+", " ", (text or "").lower())
