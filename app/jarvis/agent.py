        ...
        raw = llm.chat([
            {"role": "system",
             "content": "Ты генератор коротких кнопок-подсказок для чата с "
                        ...
                        "Предложи ТРИ продолжения СТРОГО по содержанию "
                        "ПОСЛЕДНЕГО ответа: конкретное уточнение, следующий "
                        "шаг с этим результатом или просьбу доработать. "
                        "В КАЖДОЙ фразе используй конкретное существительное "
                        "или действие ИЗ ОТВЕТА (имя файла, число, название, "
                        "героя, команду). Общих фраз вроде «расскажи "
                        "подробнее», «что дальше?», «покажи пример» — НЕ "
                        "ДОПУСКАЙ. Плохо: «расскажи подробнее». Хорошо: "
                        "«добавь уровень с боссом», «сохрани шахматы в файл». "
                        "Фразы ТОЛЬКО на русском, без английских и технических "
                        "слов. До 5 слов, от первого лица, без кавычек и "
                        "номеров. Ответь ТОЛЬКО JSON-массивом из трёх строк."},
            {"role": "user",
...
def _step_text(item: Any) -> str:
    ...
    if isinstance(item, str):
        # модель иногда помечает шаги ~~зачёркиванием~~ или **жирным**:
        # в карточке плана это мусор, шаг — чистая человеческая фраза
        return re.sub(r"~{2,}|\*{1,2}|`", "", item).strip()
