...
# Vision-модели заметно слабее следуют длинному общему system prompt, особенно
# когда изображение занимает большую часть контекста. Поэтому правила выбора
# для зрительных задач повторяются коротким system-сообщением НЕПОСРЕДСТВЕННО
# перед multimodal user message (server._chat_stream). Это не второй LLM-вызов:
# контракт едет в том же запросе к модели и лишь возвращает существующий ```ui
# pipeline, который фронтенд уже умеет превращать в интерактивные плитки.
VISION_UI_CONTRACT = """ЗРИТЕЛЬНАЯ ЗАДАЧА — ИНТЕРАКТИВНЫЙ ВЫБОР:
Если пользователь просит творчески преобразовать изображение (например, сделать
мем, стилизацию, постер или аватар), но не задал стиль/концепцию, НЕ выбирай за
него и НЕ перечисляй варианты обычным Markdown-списком. В этом же первом ответе
дай 3–4 уместных для конкретного кадра варианта единственным fenced-блоком ui:

```ui
tiles Стиль: вариант 1 | вариант 2 | вариант 3 | вариант 4
```

Подписи должны быть короткими и конкретными. Не добавляй кнопку «Сгенерировать»:
интерфейс сам отправит выбор и всегда покажет поле «Свой вариант». Если стиль
уже явно задан пользователем, ui-блок не нужен — сразу выполняй просьбу."""
