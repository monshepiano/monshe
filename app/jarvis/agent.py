...
def _describe_screen(data_url: str, shot: Dict[str, Any]) -> str:
    ...
    prompt = (
        "Это снимок экрана компьютера. Разрешение снимка: %sx%s пикселей.\n"
        "Опиши кратко и по делу:\n"
        "1) какое приложение/окно активно;\n"
        "2) списком — кликабельные элементы (кнопки, вкладки, пункты меню, "
        "поля ввода, элементы бокового списка) с их подписями и координатами "
        "центра в формате «Название — (x, y)»;\n"
        "3) если виден список диалогов или вкладок — перечисли их подписи сверху вниз "
        "с координатами.\n"
        "Координаты считай в пикселях этого снимка от левого верхнего угла. "
        "Не выдумывай элементы, которых не видишь."
    ) % (w or "?", h or "?")
    try:
